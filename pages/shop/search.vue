	...
	<view>
		<u-navbar back-icon-size="35" height="50" :border-bottom="false" :background="{backgroundColor:'#FFF'}" title="">
			<u-search @change="inputCondition" @search="screenGoods(subTabsIndex,true)" @custom="screenGoods(subTabsIndex,true)"
			 margin="0 20rpx" :action-style="{color:'#0077FF',fontSize:'32rpx'}" :value="condition" placeholder="请输入搜索内容"></u-search>
		</u-navbar>
		<view class="subTabs">
			<u-tabs :is-scroll="false" gutter="570" bar-width="60" inactive-color="#333" bar-height="4" bg-color="#FFF" height="70"
			 name="title" duration="0.3" active-color="#0077FF" :list="subTabs" :current="subTabsIndex" @change="screenGoods($event,true)"></u-tabs>
		</view>
		...
		<view class="goodsList">
			<view @click="lookDetails(item)" class="goodsitem" v-for="(item, i) in goodsDetailsData" :key="i">
				...
				<view class="info">
					...
					<view>
						<u-tag shape="circle" bg-color="#FF3E77" color="#FFF" border-color="#FF3E77" size="mini" :text="item.coupon_amount+'元卷'" />
					</view>
				</view>
			</view>
		</view>
