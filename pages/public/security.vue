<template>
	<view class="main">
		...
		<view class="upload u-padding-30">
			<view class="u-text-center"><text>请上传身份证正反面</text></view>
			<view class="slot-btn" @click="upload('y')">
				<u-image @click="upload('y')" v-if="prosimg==''"  width="100%" height="100%" mode="aspectFit" src="/static/image/security_positive.png"></u-image>
				<u-image @click="upload('y')" v-if="prosimg!=''"  width="100%" mode="widthFix" :src="prosimg"></u-image>
			</view>
			<view class="slot-btn" @click="upload('n')">
				<u-image @click="upload('n')" v-if="consimg==''"   width="100%" height="100%" mode="aspectFit" src="/static/image/security_back.png"></u-image>
		...
		<view class="foot">
			<view>
				<u-button shape="circle" :hair-line="false" :custom-style="subbtn" @click="sub">提交</u-button>
			</view>
			<view class="u-line-2 u-font-24 u-type-info u-margin-top-60">
	...
	export default {
		...
		data() {
			return {
				...
				subbtn: {
					background: '#0077FF',
					boxShadow: '0px 5px 16px 0px rgba(44, 100, 222, 0.22)',
					color: '#FFF',
					height:'90rpx',
					border:'none'
				}
