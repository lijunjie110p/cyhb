<template>
	<view class="main">
		...
		<view v-if="tabsIndex==0" v-for="(item,index) in list" :key="index">
			<view class="item">
				<view>
				...
				<view class="content">
					...
					<view class="u-flex u-row-between u-margin-top-20" style="width: 400rpx;">
						<view class="u-flex u-col-center" @click="showSharePopup(item)">
							<u-icon size="36" color="#666" name="zhuanfa"></u-icon>
							<text class="u-type-info-dark u-font-28 u-margin-left-10">分享素材</text>
						</view>
						<view class="u-flex u-col-center" @click="download(previewPic[index])">
							<u-icon size="36" color="#666" name="download"></u-icon>
							<text class="u-type-info-dark u-font-28 u-margin-left-10">下载素材</text>
						</view>
					</view>
				</view>
		...
		<u-waterfall v-if="tabsIndex==1" v-model="list" ref="uWaterfall">
			<template v-slot:left="{leftList}">
				<view class="dt-item" v-for="(item,index) in leftList" :key="index">
					<u-image @click="previewImage(index>0?index+index:0)" :src="item.imgurl" width="100%" mode="widthFix"></u-image>
				</view>
			</template>
			<template v-slot:right="{rightList}">
				<view class="dt-item" v-for="(item,index) in rightList" :key="index">
					<u-image @click="previewImage(index>0?index+index+1:index+1)" :src="item.imgurl" width="100%" mode="widthFix"></u-image>
				</view>
			</template>
		</u-waterfall>
				...
				if (res.data.status == 1) {
					...
					if (this.tabsIndex == 1) {
						for (let j in this.list) {
							this.previewPic.push(this.list[j].imgurl)
						}
						return;
					}
					for (let j in this.list) {
						this.applyWorkMark(j, this.mycode.code_img)
					}
				} else {
					uni.showToast({
						title: res.data.info,
				...
				uni.getImageInfo({
					...
					success(e) {
						...
						uni.getImageInfo({
								...
								ctx.draw(true, function() {
									uni.canvasToTempFilePath({
										...
										success: function(res1) {
											that.previewPic.push(res1.tempFilePath);
										},
										fail: function(err) {
											console.log(err)
										}
							...
							fail: (err1) => {
								console.log('getImageInfo2：' + err1)
								that.$u.toast(err1)
							}
						})
					},
					fail: (err2) => {
						console.log('getImageInfo1：' + err2)
						that.$u.toast(err2)
					}
				})
			}
