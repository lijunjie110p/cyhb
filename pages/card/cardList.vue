			...
			<view v-if="card_type=='CC'">
				<view @click="selectCard(item,index)" class="card" v-for="(item,index) in card" :key="index">
					<view class="select-icon">
						<u-icon v-if="selectIndex == index" name="checkmark-circle-fill" size="50" color="#0078FF"></u-icon>
						<u-icon v-else name="checkmark-circle" size="50" color="#0078FF"></u-icon>
					</view>
					<view class="u-flex u-col-center">
						...
						<view class="u-margin-left-35">
							<view><text class="u-font-36">{{item.bank_name}}</text></view>
							<view><text class="u-font-26 u-type-info">{{item.card_type=='CC'?'信用卡':'储蓄卡'}}</text></view>
						</view>
					</view>
					<view class="u-margin-top-10 number">
...
<script>
	...
	export default {
		...
		onLoad(parms) {
			...
			this.channel_id = parms.channel_id;
		...
		methods: {
			...
			details() {
				...
				if (this.card_type == 'DC' && this.source == 'withdraw') {
					...
					uni.navigateTo({
						url: '../gathering/channel?source=channel&card_id=' + item.card_id
					})
				} else if (this.card_type == 'CC' && this.source == 'payWallet') {
					uni.navigateTo({
						url: '../wallet/payBalance?id=' + this.channel_id+'&bid='+item.card_id
					})
				} else if (this.card_type == 'CC' && this.source == 'gathering' || this.card_type == 'CC' && this.source == 'pos' ||
					this.card_type == 'CC' && this.source == 'huabei') {
					uni.navigateTo({
...
<style lang="scss">
	...
	.botttom-addbtn {
		...
		border-radius: 50rpx;
		background: #0078FF;
		box-shadow: 1px 5px 16px 0px rgba(45, 101, 222, 0.22);
		color: #FFF;
	}
